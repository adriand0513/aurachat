...
# ── Core Reply Logic ────────────────────────────────────────────────────────
async def generate_reply_logic(convo_id: str):
    ...
    try:
        ...
        if history:
            for msg in reversed(history):
                if msg.get("role") == "user" and "timestamp" in msg:
                    try:
                        last_time = datetime.fromisoformat(str(msg["timestamp"]).replace("Z", "+00:00"))
                        time_gap_minutes = int((datetime.now(ZoneInfo("UTC")) - last_time).total_seconds() / 60)
                        if time_gap_minutes > 60:
                            if time_gap_minutes < 1440:
                                silence_note = "The user just came back after several hours. Respond naturally and casually."
                            else:
                                silence_note = "The user just came back after more than a day. Greet him warmly."
                    except:
                        pass
                    break
        ...
        if has_emotion and random.random() < 0.75 and bubbles:
            try:
                ...
                logger.error(f"{log_prefix} ElevenLabs FAILURE: {e}")
        ...
        return {"replies": ["Sorry... I got distracted for a sec"], "voice_note": ""}
...
async def chat_endpoint(body: Dict[str, str] = Body(...)):
    ...
    return await generate_reply_logic(convo_id)
