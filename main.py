...
async def generate_reply(body: Dict[str, str] = Body(...)):
    convo_id = body.get("convo_id")
    user_message = body.get("message")   # ← New: user message passed directly
    ...
    try:
        ...
        # Save user message immediately if provided
        ...
        # CSV log
        if bubbles:
            last_user = user_message or (next((msg["content"] for msg in reversed(history) if msg["role"] == "user"), ""))
            log_to_csv(
                convo_id=convo_id,
        ...
        logger.info(f"Successfully generated {len(bubbles)} bubbles for {convo_id}")
